<template>
  <div class="callarea">
    <div v-show="switchTem == false">
      <div class="top">
        <affix :label="$t('report.relaycallSea')"></affix>
        <div class="search">
          <template>
      ...
      <header-option v-if="isShow" :show="isShow" :option="headOptionsShow" type="call_report_relay_area" v-on:close="close"></header-option>
    </div>
    <openrelayarea :cityName = "{cityName, queryOne, checkedServiceNos}" @flag="show" v-if="switchTem == true"></openrelayarea>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      search () {
        ...
        let data = {reportType: 'call_report_relay_area'}
        let self = this
        this.provinces.forEach(function (item) {
          if (self.checkedProvinces.indexOf(item) >= 0) {
            self.query['Province_' + item] = true
        ...
        this.$store.dispatch('queryCallRelayAreaReport', data).then(() => {
          this.headOptions = this.$store.state.report.call.relayAreaReport.tableHeader
          this.sum = [{Province: '合计'}]
          let tempData = deepClone(this.$store.state.report.call.relayAreaReport.data)
