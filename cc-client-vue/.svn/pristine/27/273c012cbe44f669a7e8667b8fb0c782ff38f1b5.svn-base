<template>
  <div class="channel" v-loading="loading">
    <div class="top">
      <affix :label="$t('report.imReportCsrChannelSearch')"></affix>
      <div class="search">
        <template>
      ...
      <div class="tab">
        <div class="deploy">
          ...
          <el-tooltip class="item" effect="dark" placement="right" popper-class="reportTip">
            <div slot="content">
              {{$t('report.participationRate')}} <br>
              {{$t('report.sationTotal')}}
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      search () {
        ...
        this.loading = true
        this.$store.dispatch('queryImChannelCsrReport', data).then(() => {
          // 表头
          this.headOptions = deepClone(this.$store.state.report.im.channelCsrReport.tableHeader) || this.headOptions

          this.channelOptions = deepClone(this.$store.state.report.im.channelCsrReport.channels) || this.channelOptions
          this.serverData = deepClone(this.$store.state.report.im.channelCsrReport.data) || this.serverData
          this.loading = false
        })
      },
      exportXls () {
      ...
      confirm () {
        ...
        exportReportPdf({title: '在线咨询渠道满意度报表' + searchTime(this.query), id: this.id})
      }
    },
    beforeMount () {
      this.$nextTick(() => {
