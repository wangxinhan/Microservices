      ...
      <div class="tab">
        <div class="deploy">
          ...
          <el-tooltip class="item" effect="dark" placement="right" popper-class="reportTip">
            <div slot="content">
              {{$t('report.accessConverNuuber')}} <br>
              {{$t('report.finishConverNumber')}} <br>
              {{$t('report.conSationFin')}} <br>
              {{$t('report.conTurnOut')}} <br>
              {{$t('report.averSationTime')}} <br>
              {{$t('report.averResponseTime')}}
            </div>
  ...
  export default {
    ...
    computed: {
      tableData () {
        ...
        data.forEach(item => {
          ...
          item.sessionCompleteRate = `${item.sessionCompleteRate}%`
          item.sessionRollOutRate = `${item.sessionRollOutRate}%`
        })
      ...
      totalData () {
        ...
        // 会话完成率
        total.sessionCompleteRate = total.sessionAccessCount + total.sessionRollInCount > 0 ? Math.round(((total.sessionCompleteCount + total.sessionRollOutCount) / (total.sessionAccessCount + total.sessionRollInCount + (total.inviteSessionCount || 0))) * 100) : 0
        total.sessionCompleteRate = `${total.sessionCompleteRate}%`
        // 会话转出率
        total.sessionRollOutRate = total.sessionAccessCount > 0 ? Math.round((total.sessionRollOutCount / total.sessionAccessCount) * 100) : 0
        total.sessionRollOutRate = `${total.sessionRollOutRate}%`
      ...
      highchartsOptions () {
        ...
        headerConfig.forEach(item => {
          ...
          } else if (item.name === 'sessionCompleteRate' || item.name === 'sessionRollOutRate') {
    ...
    methods: {
      ...
      search () {
        ...
        this.loading = true
        this.$store.dispatch('queryImAgentReport', data).then(() => {
          // 表头
          this.headOptions = deepClone(this.$store.state.report.im.agentReport.tableHeader) || this.headOptions
          this.headOptionsShow = deepClone(this.$store.state.report.im.agentReport.tableHeader) || this.headOptionsShow

          this.serverData = deepClone(this.$store.state.report.im.agentReport.data) || this.serverData
          this.loading = false
        })
      },
      exportXls () {
      ...
      close (data) {
        if (data) {
          this.headOptions = deepClone(this.$store.state.report.im.agentReport.tableHeader)
        }
        this.headOptionsShow = deepClone(this.$store.state.report.im.agentReport.tableHeader)
      ...
      confirm () {
        ...
        exportReportPdf({title: '在线咨询座席工作量报表' + searchTime(this.query), id: this.id})
      }
    },
    beforeMount () {
